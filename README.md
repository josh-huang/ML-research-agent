...
| **Ours — DIN (k=32, + CWM aux + videoside)** | **0.6660** | **0.5315** | **0.5988** | **+0.0042** |
| oracle ceiling | 1.0000 | 0.7289 | 0.8645 | +0.2699 |

`primary = mean(GAUC, nDCG@5)`, evaluated on the held-out test split by the *official*
`evaluate.py` (imported, never modified). The gain is **variance-bounded, not a one-shot**: a
3-seed calibration on the best config gives test primary 0.5988 / 0.5988 / 0.5990 (mean 0.5989,
std ≈ 0.0001). The agent reproduced the baseline, explored the headroom, and converged
**fully autonomously** — 0 human interventions, 383,772 tokens, 0.0535 GPU-hours, 0 errors, in
7 iterations.
...
1. **Floor (hand-built, Phase 3).** A small, verified-strong model scaffold is built by hand
   *first*, so the score never depends on the agent getting lucky. This is the `DIN(k=32)`
   anchor — `valid 0.6053 / test 0.5988` — a real +0.0042 over baseline.
...
cite EDA evidence for a falsifiable hypothesis. In the autonomous run the agent reproduced the
baseline, locked the plateau, ran a 3-seed noise calibration, and — correctly — concluded the
frontier is saturated, matching the hand-built floor's conclusion.
...
| CWM **censored aux** (one-sided watch-time regression as an *aux* loss) | lifts DIN only, weight-insensitive | ✅ keep |
...
The full run-log is in `run_logs/run_log.jsonl`; a live monitoring dashboard runs via
`streamlit run dashboard/app.py`; the written project report is `docs/REPORT.pdf`.
...
|---|---|---|
| wall-clock | **339 s** (~5.6 min) | 50-iteration / 6 h cap never approached; converged in 7 iters |
| tokens | **383,772** (input+output+cache) | system prompt cached once; thinking disabled; reflector metric-precise |
...
If we had more budget, the directions we'd push next are the two signal sources the agent's EDA
already ranked but which sit outside the current config-only action space: (1) true sequential
user-history density (`hist_len` / `time_since_last`, rank-relevance up to 0.89), and (2)
duration-aware negative-sampling reweighting — both requiring harness-level code, not config flags.
